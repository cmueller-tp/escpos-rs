...
pub use self::printer_profile::{PrinterConnectionData, PrinterProfile, PrinterProfileBuilder};

mod printer_model;
mod printer_profile;

use crate::command::{Command, Font};
use crate::{Error, EscposImage, Formatter, Instruction, PrintData};
...
enum PrinterConnection {
    ...
    Network {
        /// TCP stream object
        stream: TcpStream,
        /// Time to wait before giving up writing to the socket
        timeout: std::time::Duration,
    },
    Terminal,
}
...
impl Printer {
    ...
    pub fn new(printer_profile: PrinterProfile) -> Result<Option<Printer>, Error> {
        ...
        match printer_profile.printer_connection_data.clone() {
            ...
            PrinterConnectionData::Network { host, timeout } => {
                let addr: SocketAddr = host
                    .parse()
                    .map_err(|err: AddrParseError| Error::NetworkError(err.to_string()))?;
                let stream = TcpStream::connect_timeout(&addr, timeout)
                    .map_err(|err| Error::NetworkError(err.to_string()))?;
                Ok(Some(Printer {
                    printer_connection: PrinterConnection::Network { stream, timeout },
                    printer_profile,
                    font_and_width,
                    formatter,
                    space_split: false,
